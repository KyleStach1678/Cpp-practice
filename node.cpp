...
std::tuple<Node*,Node*,Node*,Node*> Node::parse(std::string filename) {
	...
	while (!file.eof()) {
		...
		info[node] = std::tuple<int,int,int,int>(leftName, leftWeight, rightName, rightWeight);
	}
...
/*
** Node::populate
** node_map: a map with the key as an integer of the node's name and the value as a
** 		tuple of four elements with the first two being the left child's name and weight
** 		respectively and the last two being those of the right child
** Populates the node's children based on a map of values corresponding to connections
*/
std::tuple<Node*,Node*,Node*> Node::populate(Node::ParsedMap& node_map, int src_name, int dst_name) {
	...
	if (it != node_map.end()) {
		...
		if (leftName > 0) {
			...
			std::tie(srctmp, dsttmp, commontmp) = this->left->populate(node_map, src_name, dst_name);
			if (srctmp != nullptr) std::get<0>(ret) = srctmp;
			if (dsttmp != nullptr) std::get<1>(ret) = dsttmp;
			if (commontmp != nullptr) std::get<2>(ret) = commontmp;
		}
		else {
			this->left = nullptr;
		}
		...
		if (rightName > 0) {
			...
			Node* srctmp, *dsttmp, *commontmp;
			std::tie(srctmp, dsttmp, commontmp) = this->right->populate(node_map, src_name, dst_name);
			if (srctmp != nullptr) std::get<0>(ret) = srctmp;
			if (dsttmp != nullptr) std::get<1>(ret) = dsttmp;
			if (commontmp != nullptr) std::get<2>(ret) = commontmp;
		}
		...
		if (std::get<0>(ret) != nullptr && std::get<1>(ret) != nullptr && std::get<2>(ret) == nullptr) {
			std::get<2>(ret) = this;
		}
...
** Returns the node and its children, recursively printed, in the following format:
** 1:-1
**   2:5
**     4:7
**       6:2
